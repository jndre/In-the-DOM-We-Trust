...
def compress_graph(webpage_folder_path, node_file=constantsModule.NODE_INPUT_FILE_NAME, edge_file=constantsModule.RELS_INPUT_FILE_NAME, edges_file_dynamic=constantsModule.RELS_DYNAMIC_INPUT_FILE_NAME):
    ...
    try:
        cmd1="pigz --force %s"%(os.path.join(webpage_folder_path, node_file))
        cmd2="pigz --force %s"%(os.path.join(webpage_folder_path, edge_file))
        cmd3="pigz --force %s"%(os.path.join(webpage_folder_path, edges_file_dynamic))
        ...
        logger.error(f"An error occurred while compressing hpg graph: {e}")

def decompress_graph(webpage_folder_path, new_webpage_folder_path, node_file=constantsModule.NODE_INPUT_FILE_NAME, edge_file=constantsModule.RELS_INPUT_FILE_NAME, edges_file_dynamic=constantsModule.RELS_DYNAMIC_INPUT_FILE_NAME):

    cmd1="pigz -dc %s.gz > %s"%(os.path.join(webpage_folder_path, node_file), os.path.join(new_webpage_folder_path, node_file))
    cmd2="pigz -dc %s.gz > %s"%(os.path.join(webpage_folder_path, edge_file), os.path.join(new_webpage_folder_path, edge_file))
    cmd3="pigz -dc %s.gz > %s"%(os.path.join(webpage_folder_path, edges_file_dynamic), os.path.join(new_webpage_folder_path, edges_file_dynamic))
