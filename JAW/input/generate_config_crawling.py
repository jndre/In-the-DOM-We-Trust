...
"""
  ...
  Usage:
  ------------
  $ python3 -m input.generate_config_crawling --top=500 --offset=0 --parallel=5 --type=firefox
...
testbed:
  ...
  sitelist: /dataflow_analysis_scripts/webpages_with_highest_NO_dynamic_taintflows.json
  from_row: 0
  to_row: 5
...
crawler:
  ...
  # start and end indexes of the urls in Tranco list
  start_idx: %s
  end_idx: %s
...
dom_selectors:
  ...
  passes:
    crawling: true
    static: false
    static_neo4j_sinks: false
    static_neo4j_sources: false
    verification: false
"""
...
def main():
    ...
    RUN_ALL_FILE_PATH = os.path.join(BASE_DIR, 'run_all_crawling.sh');
    STOP_ALL_FILE_PATH = os.path.join(BASE_DIR, 'stop_all_crawling.sh')
    ...
    with open(RUN_ALL_FILE_PATH, 'w+') as fp1:
        with open(STOP_ALL_FILE_PATH, 'w+') as fp2:
            for i in range(parallel_instances):
                ...
                from_row =  str(offset + i*int_testbed_range)
                to_row =  str(offset + (i+1)*int_testbed_range - 1)

                http_port = http_port_offset + i*3
                bolt_port = http_port + 2

                filename = 'b'+ str(i+1) + '.conf.yaml'
                file_path = os.path.join(os.path.join(BASE_DIR, 'input/crawling_configs'), filename)
                with open(file_path, 'w+') as fd:
                    fd.write(base_conf%(from_row, to_row, http_port, bolt_port))

                fp1.write("screen -dmS s%s bash -c 'python3 -m run_pipeline --conf ./input/crawling_configs/%s 2>&1 | tee .CRAWLING_LOGFILE%s; exec sh'\n"%(str(i+1),filename, str(i+1)))
