...
testbed:
  ...
  sitelist: /dataflow_analysis_scripts/new_data_3random_webpages_list.json
  from_row: %s
  to_row: %s
...
dom_selectors:
  ...
  passes:
    crawling: false
    static: true
    static_neo4j_sinks: true
    static_neo4j_sources: true
    verification: false
"""
...
def main():
    ...
    RUN_ALL_FILE_PATH = os.path.join(BASE_DIR, 'new_run_all.sh');
    STOP_ALL_FILE_PATH = os.path.join(BASE_DIR, 'new_stop_all.sh')
    ...
    with open(RUN_ALL_FILE_PATH, 'w+') as fp1:
        with open(STOP_ALL_FILE_PATH, 'w+') as fp2:
            for i in range(parallel_instances):
                ...
                filename = 'new_b'+ str(i+1) + '.conf.yaml'
                ...
                fp1.write("screen -dmS s%s bash -c 'python3 -m run_pipeline --conf ./input/configs/%s 2>&1 | tee new_LOGFILE%s; exec sh'\n"%(str(i+1),filename, str(i+1)))
