...
def API_neo4j_prepare(csv_absolute_path, nodes_name=constantsModule.NODE_INPUT_FILE_NAME, relationships_name=constantsModule.RELS_INPUT_FILE_NAME, load_dom_tree_if_exists=True):
    ...
    NEO4J_IMPORT_COMMAND = NEO4J_IMPORT_COMMAND.encode('utf-8')
...
def neoadmin_import_db_instance(ineo_db_name, neo4j_db_name, nodes_file, rels_file, rels_dynamic_file=None):
    ...
    try:
        ...
        LOGGER.info("during execution of neoadmin_import_db_instance: %s"%(e))
        if rels_dynamic_file:
            command = "NEO4j_ADMIN import --database={0} --nodes={1} --relationships={2},{3} --delimiter='~' --skip-bad-relationships=true --multiline-fields=true --skip-duplicate-nodes=true --bad-tolerance=100000 --ignore-extra-columns=true --skip-bad-entries-logging=true".format(neo4j_db_name, nodes_file, rels_file, rels_dynamic_file)
        else:
            command = "NEO4j_ADMIN import --database={0} --nodes={1} --relationships={2} --delimiter='~' --skip-bad-relationships=true --multiline-fields=true --skip-duplicate-nodes=true --bad-tolerance=100000 --ignore-extra-columns=true --skip-bad-entries-logging=true".format(neo4j_db_name, nodes_file, rels_file)
        ...
        try:
            ...
            Logger.info("second try also did not work!")
