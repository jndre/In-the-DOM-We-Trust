...
def pretty_get_program_slices(slices):
    ...
    for i in range(len(slices)):
        ...
        outputs.append("\t\tslice %s- (loc: %s ): %s\n"%(i+1, location, s[0]))
...
def _get_return_value_of_function(tx, function_name, cfg_node_id):
    query = """
    MATCH (n {Id: $cfg_node_id})-[:AST_parentOf*]->(function_name {Type: 'Identifier', Code: $function_name})
    ...
    results = tx.run(query, cfg_node_id=cfg_node_id, function_name=function_name)
...
def get_value_of_return_value_of_function_from_context(tx, function_name, cfg_node_id, max_varname_value_depth=30, max_func_depth=3):
    sink_cfg_node = None
    return_variables = None
    if function_name and cfg_node_id:
        sink_cfg_node, return_variables = _get_return_value_of_function(tx, function_name, cfg_node_id)

    out_values = []

    if sink_cfg_node and return_variables:
        for vname in return_variables.keys():
            if max_varname_value_depth > 0:
                v = _get_varname_value_from_context(tx, vname, sink_cfg_node['Id'], max_varname_value_depth=(max_varname_value_depth - 1), max_func_depth=max_func_depth)
                for x in v:
                    if x not in out_values:
                        out_values.append(x)
...
def _get_varname_value_from_context(tx, varname, context_node_id, PDG_on_variable_declarations_only=False, context_scope='', max_varname_value_depth=30, max_func_depth=3):
    """
    Description:
    -------------
    ...
    for item in results:
        ...
        for iteratorNode in currentNodes:
                    ...
                    if match_signature:
                        ...
                        for nid in varname_values_within_call_expressions:
                            ...
                            if each_argument['Type'] == 'Literal':
                                ...
                                if max_varname_value_depth > 0:
                                    recurse= _get_varname_value_from_context(tx, each_argument['Value'], top_level_of_call_expr['Id'], context_scope=context_id_of_call_scope, max_varname_value_depth=(max_varname_value_depth - 1), max_func_depth=max_func_depth)
                                ...
                                if max_varname_value_depth > 0:
                                    recurse= _get_varname_value_from_context(tx, top_most, top_level_of_call_expr['Id'], context_scope=context_id_of_call_scope, max_varname_value_depth=(max_varname_value_depth - 1), max_func_depth=max_func_depth)
                                    out_values.extend(recurse)
                                ...
                                if additional_identifiers is not None and max_varname_value_depth > 0:
                                    for each_additional_identifier in additional_identifiers:
                                        top_level_of_call_expr = QU.get_ast_topmost(tx, {'Id': call_expr_id})
                                        recurse= _get_varname_value_from_context(tx, each_additional_identifier, top_level_of_call_expr['Id'], context_scope=context_id_of_call_scope, max_varname_value_depth=(max_varname_value_depth - 1), max_func_depth=max_func_depth)
                                        out_values.extend(recurse)
                            else:
                                ...
                                call_expr_id = _get_node_id_part(nid)
                                top_level_of_call_expr_for_functions_as_argument = QU.get_ast_topmost(tx, {'Id': call_expr_id})
                                if max_func_depth > 0:
                                    for func_arg in each_argument['ResolveIdentifiers']:
                                        function_return_value_slices = get_value_of_return_value_of_function_from_context(tx, func_arg, top_level_of_call_expr_for_functions_as_argument['Id'], max_varname_value_depth, max_func_depth=(max_func_depth - 1))

                                        for x in function_return_value_slices:
                                            if x not in out_values:
                                                out_values.append(x)
                            ...
                            if additional_identifiers is not None:
                                if 'ThisExpression' in additional_identifiers:
                                    ...
                                    for item in pointer_resolutions['methods']:
                                        ...
                                        if owner_item != '' and owner_item is not None and owner_item!= constantsModule.WINDOW_GLOBAL_OBJECT and owner_item['Type'] == 'Identifier' and max_varname_value_depth > 0:
                                            recurse_values = _get_varname_value_from_context(tx, tree_owner_exp, owner_top['Id'], PDG_on_variable_declarations_only=True, max_varname_value_depth=(max_varname_value_depth - 1), max_func_depth=max_func_depth)
            ...
            out_values.append([code_expr, literals, idents, loc, iteratorNode['Id']])
            ...
            for new_varname in new_varnames:
                ...
                for definition in call_definition_result:
                    ...
                    if is_func_call and max_func_depth > 0:
                        top_level_of_call_expr_for_functions_as_argument = QU.get_ast_topmost(tx, {'Id': new_varname_id})
                        function_return_value_slices = get_value_of_return_value_of_function_from_context(tx, new_varname, top_level_of_call_expr_for_functions_as_argument['Id'], max_varname_value_depth, max_func_depth=(max_func_depth - 1))
                        for x in function_return_value_slices:
                            if x not in out_values:
                                out_values.append(x)
                ...
                if max_varname_value_depth > 0:
                    v = _get_varname_value_from_context(tx, new_varname, contextNode['Id'], context_scope = context_scope, max_varname_value_depth=(max_varname_value_depth - 1), max_func_depth=max_func_depth)
                    out_values.extend(v)
