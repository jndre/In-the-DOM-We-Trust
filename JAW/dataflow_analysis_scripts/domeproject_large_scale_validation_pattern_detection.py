...
output_csv = "StaticAnalysis_with_filters_without_dots.csv"
...
for filter_type in filters:
    all_patterns.extend(filters[filter_type])
...
with open(output_csv, mode="w", newline="") as csv_file:
    ...
    prefixed_headers = ["Source", "Sink", "loc", "JS_file", "url"]
    all_headers = []
    all_headers.extend(prefixed_headers)
    all_headers.extend(all_patterns)
            ...
            with open(sinks_flows_file, "r") as json_file:
                ...
                for flow in flows:
                    ...
                    if matched_strings and not filter_objects_with_data_flow(flow):
                        #aggregated_code = ""
                        #sink_code = flow.get("sink_code", [])
                        #aggregated_code += sink_code
                        program_slices = flow.get("program_slices", {})

                        for _, arg_flow in program_slices.items():
                            sem_type_check = False
                            current_source = ""
                            for sem_type in arg_flow["semantic_types"]:
                                if sem_type in semantic_type_criteria:
                                    sem_type_check = True
                                    current_source = sem_type
                                    break

                            if not sem_type_check:
                                continue
                            ...
                            slice_depth = 0
                            for slicee in arg_flow["slices"]:
                                slice_depth += 1
                                for sub in redundant_substrs:
                                    current_sc = slicee["code"]
                                    current_sc = current_sc.replace(sub, "")
                                aggregated_code += current_sc
                            ...
                            for pattern in all_patterns:
                                filter_vector.append(aggregated_code.count(pattern))
                            ...
                            final_row = []
                            final_row.extend([current_source, sink_type, loc, js_file, url])
                            final_row.extend(filter_vector)
